...
> The system prompt, user prompt template, validation rules, and fallback content for AI-generated reports under the V2 (21-capability) framework. Refining this file refines report quality without touching code. Bump the version on every meaningful prompt change. The v0.3 file (`14_ai_prompts.md`, written when the framework had 15 capabilities) is preserved alongside for historical reference.
...
```
You are a senior consultant at Forefront Consulting writing a brief board-grade report for a client's leadership team. The client has just completed an Endurance Assessment that measures the organization across three pillars — Agility (sense and move), Toughness (absorb and hold), and Resilience (recover and renew) — and 21 underlying capabilities.
...
- `{{capability_bands_summary}}` — for individual rows, format as `"3 in Critical Gap, 4 in Needs Work, 6 in Solid, 2 in Strong"` rather than per-capability listings (keeps prompt size manageable). Note: under V2 the tally is over 21 capabilities per respondent, so totals can range up to 21.
...
| `max_tokens` (or equivalent) | 2400 | V2 adds 6 net new capabilities to the prompt list; bumped from 2000 to keep headroom. Typical responses are still ~500 tokens. |
...
| Each focus area has 1 to 3 `observations` (each non-empty string) *(v0.3)* | Retry once |
| Each focus area has 1 to 3 `actions` (each non-empty string) *(v0.3)* | Retry once |
| Each observation ≤ 30 words *(v0.3)* | Truncate at word boundary; warn in audit log |
...
Static strings keyed by the team overall band (same as the band-keyed interpretation strings in `03_scoring_and_bands - V2.md` section 5). Pre-pended with a sample-size disclaimer when applicable.
...
The baseline action items in `04_recommendations - V2.md` section 3, used verbatim. No filter adaptation in fallback mode.
...
Rough token estimates per call (using GPT/Gemini-equivalent tokenizers). V2 adds 6 new capabilities to the per-pillar list and bumps the individual-row tally fields from 15-wide to 21-wide.

| Component | Tokens |
|-----------|--------|
| System prompt | ~620 |
| User prompt — baseline (no individuals) | ~950 |
| User prompt — individuals section (per respondent ~95 tokens) | 95 × N |
| Output | ~500 |
| **Total per call (10 respondents)** | ~2520 in + ~500 out = ~3020 |
| **Total per call (50 respondents)** | ~6320 in + ~500 out = ~6820 |
...
- Respondent names (collected as required during demographics; stripped before any prompt build via `src/lib/ai/strip-names.ts`)
- Respondent access codes
- Email addresses (we don't collect these in v1)
...
| Version | Date | Change |
|---------|------|--------|
| 0.4 | 2026-05-19 | **V2 framework alignment.** System prompt phrase "15 underlying capabilities" → "21". User prompt now lists 7 capabilities per pillar (was 5). `capability_bands_summary` tally is now over 21 capabilities (was 15). § 6 token budget updated. § 5.2 cross-reference re-pointed to `04_recommendations - V2.md`. `CURRENT_PROMPT_VERSION` bumped from 3 → 4 in `src/lib/ai/prompt.ts`. **Cache handling:** the V2 question-set reset (`prisma/sql/007_v2_questions_reset.sql`) wipes the entire `GeneratedReport` table because capability keys changed — so existing rows from prompt v1/v2/v3 are gone too, and the v0.3 stale-version notice flow is moot for older versions. |
