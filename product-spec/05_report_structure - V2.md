...
> The on-screen and PDF report layout under the V2 (21-capability) framework. Sections, ordering, content, and visual treatment. The report is the most important surface of the product — designed to be presentable in a client boardroom without further formatting. The v0.2 file (`05_report_structure.md`, written for the 15-capability framework with 3×5 grid) is preserved alongside for historical reference; this file is the canonical source for V2.
...
- One-line interpretation:
  ...
  - Static band-keyed string from `03_scoring_and_bands - V2.md` section 5 (when no AI)
...
**7 capability rows** within each column (up from 5 in v0.1):
...
> **Visual impact note:** the V2 grid is 3 columns × 7 rows (21 capabilities total) versus v0.1's 3 × 5 (15). The capability profile section will be ~40% taller on desktop. Other sections are unchanged in size.
...
> Future variant (v3): admin can toggle ordering to show by spread descending — useful when divergence is the lead finding.
...
- **Two action items** (numbered 1 and 2):
  ...
  - From baseline action items in `04_recommendations - V2.md` (otherwise)
...
If fewer than 5 capabilities exist (will not happen in v1 — there are always 21 under V2), or the data is incomplete, show whatever capabilities exist with appropriate ranks.
...
### 6.2 Capability matrix (21 capabilities × N respondents)

Below the per-respondent row table: a heatmap where each cell shows the band color (no numeric value). Rows = capabilities (in canonical order, 21 of them), columns = anonymized respondents (letters).
...
- Each pillar's column from Section 2 starts on a new page (3 pillar pages, each now listing 7 capabilities)
...
- Sections rendered as twin columns:
  ...
  - Capability profile (twin score bars per capability, color-coded — 21 capability pairs now)
...
| Version | Date | Change |
|---------|------|--------|
| 0.3 | 2026-05-19 | **V2 framework alignment.** § 4.1 capability rows 5 → 7 per column (3 × 7 = 21 total). § 4.1 adds a visual-impact note flagging the ~40% taller capability profile section. § 6.2 capability matrix 15 → 21 rows. § 8 PDF pages now list 7 capabilities per pillar page. § 9.1 comparison view notes 21 capability pairs. All cross-references re-pointed to V2 spec files. |
